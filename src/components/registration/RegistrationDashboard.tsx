...
import { Bus, BookOpen, AlertCircle, ListOrdered } from 'lucide-react';
    ...
    return (
        <div className="space-y-4 mb-6">
            <div className="flex flex-col lg:flex-row gap-4">
                {/* Vehicle Stats Card */}
                <div className="flex-1 bg-white p-5 rounded-xl shadow-sm border border-purple-100 relative overflow-hidden transition-all hover:shadow-md">
                ...
                {ordinanceStats && (
                    <div className="flex-1 bg-white p-5 rounded-xl shadow-sm border border-blue-100 relative overflow-hidden transition-all hover:shadow-md">
                        <div className="absolute -top-4 -right-4 p-4 opacity-[0.03]">
                        ...
                        <div className="grid grid-cols-2 gap-6 text-sm font-medium relative z-10">
                            {/* Endowment */}
                            <div className="relative h-full flex flex-col gap-3">
                                <div className="absolute right-[-12px] top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-blue-100 to-transparent"></div>
                                <div className="text-blue-800 text-xs font-bold tracking-wider uppercase bg-blue-50 inline-block px-2 py-0.5 rounded-md text-center w-full">{t('stake.registration.form.dashboard.ordinance_endowment')}</div>
                                <div className="flex-1 space-y-2.5">
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.total_seats')}:</span>
                                        <span className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{ordinanceStats.endowment.capacity}</span>
                                    </div>
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.occupied')}:</span>
                                        <span className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{ordinanceStats.endowment.occupied}</span>
                                    </div>
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.available')}:</span>
                                        <span className={`font-bold ${ordinanceStats.endowment.remaining > 0 ? "text-green-600" : "text-red-500"}`}>{ordinanceStats.endowment.remaining}</span>
                                    </div>
                                </div>
                                <div className="flex justify-between pt-2 mt-1 border-t border-blue-50 items-center">
                                    <span className="text-gray-400 text-xs">{t('stake.registration.form.dashboard.waitlist')}:</span>
                                    <span className={`font-bold text-xs ${ordinanceStats.endowment.waiting > 0 ? "text-red-600 bg-red-50 px-1.5 py-0.5 rounded" : "text-gray-400"}`}>{ordinanceStats.endowment.waiting}</span>
                                </div>
                            </div>
                            
                            {/* Baptism */}
                            <div className="pl-2 h-full flex flex-col gap-3">
                                <div className="text-blue-800 text-xs font-bold tracking-wider uppercase bg-blue-50 inline-block px-2 py-0.5 rounded-md text-center w-full">{t('stake.registration.form.dashboard.ordinance_baptism')}</div>
                                <div className="flex-1 space-y-2.5">
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.total_seats')}:</span>
                                        <span className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{ordinanceStats.baptism.capacity}</span>
                                    </div>
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.occupied')}:</span>
                                        <span className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{ordinanceStats.baptism.occupied}</span>
                                    </div>
                                    <div className="flex justify-between items-center group">
                                        <span className="text-gray-500 text-xs">{t('stake.registration.form.dashboard.available')}:</span>
                                        <span className={`font-bold ${ordinanceStats.baptism.remaining > 0 ? "text-green-600" : "text-red-500"}`}>{ordinanceStats.baptism.remaining}</span>
                                    </div>
                                </div>
                                <div className="flex justify-between pt-2 mt-1 border-t border-blue-50 items-center">
                                    <span className="text-gray-400 text-xs">{t('stake.registration.form.dashboard.waitlist')}:</span>
                                    <span className={`font-bold text-xs ${ordinanceStats.baptism.waiting > 0 ? "text-red-600 bg-red-50 px-1.5 py-0.5 rounded" : "text-gray-400"}`}>{ordinanceStats.baptism.waiting}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        {isClosed && (
            <div className="bg-red-50 border border-red-200 text-red-800 p-4 rounded-xl shadow-sm animate-fade-in flex items-start">
