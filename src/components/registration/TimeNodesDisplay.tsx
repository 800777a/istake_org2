...
const TimeNodesDisplay: React.FC<TimeNodesDisplayProps> = ({ activeEvent, isPublic = false }) => {
    ...
    return (
        <div className="bg-white overflow-visible border-2 border-amber-200 rounded mb-4 shadow-sm animate-in fade-in slide-in-from-top-2 w-full max-w-full min-w-0">
            {/* Level 1: Header */}
            <div className="w-full flex items-center justify-between px-3 py-3 md:px-6 md:py-3 bg-gradient-to-r from-[#EAC100] via-[#FDE68A] to-[#EAC100] border-b-2 border-[#C6A300] min-w-0">
                <div className="flex items-center gap-2 md:gap-3 min-w-0">
                    <div className="p-1.5 bg-white/30 rounded shadow-sm border-2 border-white/40 text-amber-950 shrink-0">
                        <Clock className="w-4 h-4 md:w-5 md:h-5 animate-pulse" /> 
                    </div>
                    <h4 className="font-black text-amber-950 text-xs md:text-sm lg:text-base tracking-tight uppercase truncate">
                        活動重要時間節點
                    </h4>
                </div>
            </div>
            ...
            {/* Level 4: Content */}
            <div className="bg-[#FFFBEB]/50 p-2 md:p-6 w-full min-w-0">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 w-full min-w-0">
                    {nodes.map((node, idx) => (
                        <div key={idx} className="space-y-1 min-w-0 w-full group">
                            <label className="text-[9px] md:text-xs font-black text-amber-800 block truncate uppercase tracking-tighter opacity-70 group-hover:opacity-100 transition-opacity pl-1">{node.label}</label>
                            <div className="w-full p-2 md:p-2.5 bg-white border-2 border-amber-100 rounded text-[10px] md:text-xs lg:text-sm font-black text-slate-900 shadow-sm text-center md:text-left truncate overflow-hidden whitespace-nowrap transition-all group-hover:border-[#EAC100] group-hover:shadow-md group-hover:bg-[#FFFBEB]/30" title={formatDateTime(node.value)}>
