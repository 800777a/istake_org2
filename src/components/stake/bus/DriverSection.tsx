...
import { Plus, Trash2, Edit2, Search, ChevronUp, ChevronDown, User, Star, PlusCircle } from 'lucide-react';
        ...
        <section className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden animate-fade-in">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base md:text-lg font-bold text-slate-900 tracking-tight leading-none">{t('bus.driver_mgmt')}</h2>
                    </div>
                </div>
            ...
            {isOpen && (
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row justify-end gap-4">
                        ...
                        >
                            <PlusCircle size={18}/> {t('bus.add_driver')}
                        </button>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px] cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => onSort('name')}>{t('bus.driver_name')}</th>
                                    <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px]">{t('bus.phone')}</th>
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {filtered.map(d => (
                                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-slate-500 font-medium font-mono">{d.phone}</td>
                                        <td className="px-6 py-4 text-slate-500 font-medium">{d.companyName}</td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="px-2 py-1 bg-violet-50 text-violet-700 rounded text-[10px] font-bold border border-violet-100">{d.serviceCount || 0}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <div className="flex items-center justify-center gap-1.5 bg-amber-50 px-3 py-1 rounded border border-amber-100 inline-flex">
                                                ...
                                                <span className="font-bold text-amber-900">{d.avgRating?.toFixed(1) || '0.0'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => onEdit(d)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all active:scale-90"><Edit2 size={16}/></button>
                                                <button onClick={() => onDelete(d)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all active:scale-90"><Trash2 size={16}/></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
