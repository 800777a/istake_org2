...
- 側邊欄 (Sidebar)：主標題「聖殿旅行」，下方緊接「活動日期」文字置中。側邊欄在所有裝置（桌機、平板、手機、水平/垂直）下預設皆為展開狀態，展開後必須顯示「關閉 (X)」按鈕。側邊欄包含前/後台與管理權限切換按鈕。側邊欄之圖層權重 (z-index) 必須絕對高於頁首 (Header) 與頁尾 (Footer) 列。
