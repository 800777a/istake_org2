      ...
      <div className="space-y-8">
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white p-4 rounded border border-slate-100 shadow-sm space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {(billingConfig.units || []).map(unit => (
                  <div key={unit.shortName} className="px-3 py-1.5 bg-slate-50 rounded border border-slate-100 flex items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(billingConfig.identityPricings || []).length > 0 ? (
                  (billingConfig.identityPricings || []).map((p, idx) => (
                    <div key={`${p.identity}-${idx}`} className="flex items-center justify-between p-3 bg-slate-50 rounded border border-slate-100">
                      ...
                      <span className="bg-white text-blue-700 font-black text-xs px-3 py-1 rounded border border-blue-100">
                        {getPricingText(p.price.method, p.price.value)}
                      </span>
                    </div>
                  ))
                ) : (
                  <div className="col-span-2 text-center py-6 bg-slate-50 rounded border border-dashed border-slate-200 text-slate-400 text-xs italic">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(billingConfig.tripPricings || []).length > 0 ? (
                  (billingConfig.tripPricings || []).map((p, idx) => (
                    <div key={`${p.trip}-${idx}`} className="flex items-center justify-between p-3 bg-slate-50 rounded border border-slate-100">
                      <div className="flex items-center gap-3">
                        <Car className="text-slate-400 w-4 h-4" />
                        <span className="text-sm font-bold text-slate-700">{p.trip}</span>
                      </div>
                      <span className="bg-white text-cyan-700 font-black text-xs px-3 py-1 rounded border border-cyan-100">
                        {getPricingText(p.price.method, p.price.value)}
                      </span>
                    </div>
                  ))
              ...
              <div className="space-y-3">
                {(billingConfig.specialPromos || []).filter(p => p.enabled).length > 0 ? (
                  (billingConfig.specialPromos || []).filter(p => p.enabled).map(p => (
                    <div key={p.id} className="p-4 bg-slate-50 rounded border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-start gap-3">
                        <Gift className="text-slate-400 w-5 h-5 mt-1" />
                        <div>
                          <span className="font-bold text-slate-700 block mb-1">{p.name}</span>
                          <div className="flex flex-wrap gap-1">
                            {p.units && p.units.length > 0 ? p.units.map(u => <span key={u} className="px-2 py-0.5 text-[9px] font-bold rounded bg-white border border-slate-100 text-slate-500">{u}</span>) : <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-white border border-slate-100 text-slate-400">{t('stake.fee_config.all_units', '全單位')}</span>}
                            {p.identities && p.identities.length > 0 ? p.identities.map(i => <span key={i} className="px-2 py-0.5 text-[9px] font-bold rounded bg-slate-600 text-white">{i}</span>) : <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-slate-200 text-slate-500">{t('stake.fee_config.all_identities', '全身份')}</span>}
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="text-center py-6 bg-slate-50 rounded border border-dashed border-slate-200 text-slate-400 text-xs italic">
          ...
          <div>
            ...
            <div className="bg-indigo-900 text-white p-6 rounded shadow-lg relative overflow-hidden">
              <div className="relative z-10 space-y-4">
                <div className="flex items-center justify-between border-b border-white/10 pb-3">
                  ...
                  <span className="bg-amber-500 text-white px-3 py-1 rounded text-xs font-black">{billingConfig.calcStrategy === 'stack' ? t('stake.fee_config.strategy_stack_mode', '疊加模式 (Stack)') : t('stake.fee_config.strategy_min_mode', '最優模式 (Minimal)')}</span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-blue-500 text-white px-3 py-1 rounded text-xs font-black">{billingConfig.roundingToTen ? t('stake.fee_config.round_to_ten_short', '進位到十位數') : t('stake.fee_config.no_rounding', '無進位')}</span>
                </div>
              </div>
          ...
          <div className="bg-amber-50/50 p-6 rounded border border-amber-100/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="font-mono text-amber-700 text-sm bg-white p-3 rounded border border-amber-200 inline-block shadow-sm">
              ...
              <button 
                ...
                className="h-14 px-8 bg-amber-500 text-white rounded shadow-lg hover:bg-amber-600 transition-all flex items-center gap-3 self-center md:self-auto"
              >
                <Calculator size={24} />
