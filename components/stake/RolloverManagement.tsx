...
const RolloverManagement: React.FC<RolloverManagementProps> = ({ registrations, currentEventId, onRefresh }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden animate-fade-in mb-8">
            ...
            <div className="p-6 space-y-6 bg-[#F0F4F8]/10">
                ...
                <div className="flex justify-end gap-3">
                    <button 
                        ...
                        className="h-11 px-6 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-bold shadow-sm hover:bg-slate-50 transition-all flex items-center gap-2"
                    >
                        <Download size={18} className="text-blue-600" /> 
                        {t('stake.registration.export_retained_btn_text', '匯出 Roll over 名單')}
                    </button>
                    <label className={`h-11 px-6 bg-blue-600 text-white rounded-lg text-sm font-bold shadow-md hover:bg-blue-700 cursor-pointer transition-all flex items-center gap-2 ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>
                        <Upload size={18} /> 
