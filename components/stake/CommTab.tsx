        ...
        <div key={remountKey} className={`space-y-6 animate-fade-in pb-20 w-full ${viewMode === 'card' ? 'px-2' : ''}`}>
            ...
            <div className="space-y-6">
                ...
                >
                    <div className="space-y-4">
                        <div className="flex justify-end gap-2">
                            <label className="bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded text-xs flex items-center hover:bg-indigo-50 cursor-pointer font-bold shadow-sm transition-all active:scale-95">
                            ...
                            <button 
                                ...
                                className="bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded text-xs flex items-center hover:bg-indigo-50 font-bold shadow-sm transition-all active:scale-95"
                            >
                        ...
                        <div className="space-y-2">
                            {viewMode === 'table' ? (
                                        ...
                                        <table className="dense-table">
                                            ...
                                            <tbody className="divide-y divide-slate-100 bg-white">
                                                            ...
                                                            <>
                                                                ...
                                                                <td className="px-2 py-2 text-center">
                                                                    <div className="flex items-center justify-center gap-1">
                                                                        <button onClick={saveEditServicePerson} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded transition-all"><Check size={16}/></button>
                                                                        <button onClick={() => setEditingServiceId(null)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded transition-all"><X size={16}/></button>
                                                                    </div>
                                                                </td>
                                                            </>
                                                        ) : (
                                                            <>
                                                                ...
                                                                <td className="px-4 py-3 text-center">
                                                                    <div className="flex items-center justify-center gap-2">
                                                                        <button onClick={() => startEditServicePerson(p)} className="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded transition-all"><Edit2 size={14}/></button>
                                                                        <button onClick={() => triggerDeleteServicePerson(p.id)} className="p-1.5 text-rose-400 hover:bg-rose-50 rounded transition-all"><Trash2 size={14}/></button>
                                                                    </div>
                                                                </td>
                                                            </>
                                                        )}
                                                ...
                                                <tr className="bg-indigo-50/30">
                                                    <td className="p-2"><input type="number" placeholder="#" className="w-full h-9 border border-indigo-200 rounded px-1 text-center text-xs bg-white" value={newServicePerson.order} onChange={e => setNewServicePerson({...newServicePerson, order: parseInt(e.target.value)})} /></td>
                                                    <td className="p-2">
                                                    ...
                                                    <td className="p-2"><input type="text" placeholder={t('common.name', "姓名")} className="w-full h-9 border border-indigo-200 rounded px-3 text-xs bg-white" value={newServicePerson.name} onChange={e => setNewServicePerson({...newServicePerson, name: e.target.value})} /></td>
                                                    <td className="p-2 text-center">
                                                        <button 
                                                            onClick={handleAddServicePerson} 
                                                            disabled={!newServicePerson.name} 
                                                            className="bg-indigo-600 text-white h-9 px-4 rounded text-xs font-bold shadow-md hover:bg-indigo-700 disabled:opacity-50 transition-all active:scale-95 flex items-center justify-center gap-1 w-full"
                                                        >
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-1">
                                    {sortedServicePersonnel.map(p => (
                                        <div key={p.id} className="bg-white p-3 rounded border border-indigo-100 shadow-sm space-y-3">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex gap-1">
                                                    <button onClick={() => startEditServicePerson(p)} className="p-1.5 text-indigo-400 hover:bg-indigo-50 rounded transition-all"><Edit2 size={14}/></button>
                                                    <button onClick={() => triggerDeleteServicePerson(p.id)} className="p-1.5 text-rose-400 hover:bg-rose-50 rounded transition-all"><Trash2 size={14}/></button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {/* Inline Add Card */}
                                    <div className="bg-indigo-50/50 p-3 rounded border border-dashed border-indigo-300 space-y-3">
                                        ...
                                        <input type="text" placeholder={t('common.name', '姓名')} className="w-full h-8 border rounded px-2 text-[11px]" value={newServicePerson.name} onChange={e => setNewServicePerson({...newServicePerson, name: e.target.value})} />
                                        <button 
                                            onClick={handleAddServicePerson} 
                                            disabled={!newServicePerson.name}
                                            className="w-full bg-indigo-600 text-white h-8 rounded text-[11px] font-bold hover:bg-indigo-700 transition-all flex items-center justify-center gap-1"
                                        >
                ...
                >
                    <div className="space-y-6">
                        <div className="flex justify-end gap-2">
                            <button onClick={handleExport} className="bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded text-xs flex items-center hover:bg-indigo-50 font-bold shadow-sm transition-all active:scale-95"><Download className="w-3 h-3 mr-1.5"/>{t('common.export', '匯出')}</button>
                            <label className="bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded text-xs flex items-center hover:bg-indigo-50 cursor-pointer font-bold shadow-sm transition-all active:scale-95"><Upload className="w-3 h-3 mr-1.5"/>{t('common.import', '匯入')}<input type="file" className="hidden" accept=".json" onChange={handleImport}/></label>
                        </div>

                        <div className="bg-slate-50 p-3 md:p-4 rounded border border-slate-200 space-y-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
                                ...
                                <div>
                                    ...
                                    <input type="text" className="w-full h-10 border border-slate-200 rounded px-3 text-xs outline-none focus:ring-2 focus:ring-indigo-500" value={newContact.position} onChange={e => setNewContact({...newContact, position: e.target.value})} placeholder={tString('comm.position_placeholder', "例如: 主教")} />
                                </div>
                                <div>
                                    <label className="text-[10px] text-slate-400 font-bold block mb-1">{t('common.name', '姓名')}</label>
                                    <input type="text" className="w-full h-10 border border-slate-200 rounded px-3 text-xs outline-none focus:ring-2 focus:ring-indigo-500" value={newContact.name} onChange={e => setNewContact({...newContact, name: e.target.value})} placeholder={tString('common.name', "姓名")} />
                                </div>
                                <div>
                                    <label className="text-[10px] text-slate-400 font-bold block mb-1">{t('common.gender', '性別')}</label>
                                    <select className="w-full h-10 border border-slate-200 rounded px-2 text-xs bg-white outline-none focus:ring-2 focus:ring-indigo-500" value={newContact.gender} onChange={e => setNewContact({...newContact, gender: e.target.value as any})}>
                                ...
                                <div className="col-span-2 md:col-span-1">
                                    ...
                                    <input 
                                        type="tel" 
                                        className="w-full h-10 border border-slate-200 rounded px-3 text-xs outline-none focus:ring-2 focus:ring-indigo-500" 
                            ...
                            <div className="flex justify-end">
                                <button onClick={handleAddContact} className="bg-indigo-600 text-white h-10 px-6 rounded text-xs font-bold hover:bg-indigo-700 shadow-md transition-all active:scale-95 flex items-center gap-2">
                        ...
                        <div className="space-y-2">
                            {viewMode === 'table' ? (
                                                ...
                                                {staffContacts.map((c: any) => (
                                                    <tr key={c.id} className={`hover:bg-slate-50 transition-colors ${c.isAuto ? 'bg-slate-50/30' : ''}`}>
                                                        ...
                                                        <td className="px-4 py-3 text-center">
                                                            {!c.isAuto ? (
                                                                <div className="flex items-center justify-center gap-2">
                                                                    <button onClick={() => handleEditContact(c)} className="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded transition-all"><Edit2 size={14}/></button>
                                                                    <button onClick={() => handleDeleteContact(c.id)} className="p-1.5 text-rose-400 hover:bg-rose-50 rounded transition-all"><Trash2 size={14}/></button>
                                                                </div>
                                                            ) : (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-1">
                                    {staffContacts.map((c: any) => (
                                        <div key={c.id} className={`bg-white p-3 rounded border shadow-sm transition-all hover:shadow-md ${c.isAuto ? 'border-slate-100 bg-slate-50/30' : 'border-indigo-100'}`}>
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                {!c.isAuto && (
                                                    <div className="flex gap-1">
                                                        <button onClick={() => handleEditContact(c)} className="p-1.5 text-indigo-400 hover:bg-indigo-50 rounded transition-all"><Edit2 size={14}/></button>
                                                        <button onClick={() => handleDeleteContact(c.id)} className="p-1.5 text-rose-400 hover:bg-rose-50 rounded transition-all"><Trash2 size={14}/></button>
                                                    </div>
                                                )}
                                            </div>
