      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="relative bg-white rounded-[40px] shadow-2xl w-full max-w-sm overflow-hidden border-4 border-white"
          >
            {/* Title Bar */}
            <div className="px-8 py-5 bg-gray-50 border-b flex justify-between items-center">
              <div className="flex items-center">
                <div className="w-8 h-8 rounded-xl bg-blue-100 flex items-center justify-center text-blue-600 mr-3">
                  <ShieldAlert size={20} />
                </div>
            ...
            <div className="p-8">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-2 bg-gray-100 p-1 rounded-2xl">
                  <button 
                    ...
                    className={`py-2.5 rounded-xl text-xs font-black transition-all ${destination === 'download' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
                  >
                    存為檔案
                  </button>
                  <button 
                    onClick={() => setDestination('editor')}
                    className={`py-2.5 rounded-xl text-xs font-black transition-all ${destination === 'editor' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}
                  >
                    傳送至文書處理
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button 
                  onClick={onClose}
                  className="w-full py-3 text-gray-400 rounded-2xl font-black text-sm hover:text-gray-600 transition-all"
                >
                  放　棄 / Cancel
                </button>
              </div>
