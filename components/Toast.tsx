...
const Toast: React.FC<ToastProps> = ({ message, type = 'success', onClose, duration = 3000 }) => {
    ...
    const getColors = () => {
        switch (type) {
            ...
            default: return 'bg-orange-900 border-orange-400';
        }
    };
        ...
        <AnimatePresence>
            {message && (
                <motion.div 
                    ...
                    className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-[999] ${getColors()} text-white px-6 py-3 rounded-2xl font-bold shadow-2xl border-2 flex items-center gap-3 cursor-pointer min-w-[300px] justify-between`}
                    onClick={onClose}
                >
